...
class SysUserService:
    ...
    def update_sys_user(user_id, form_data):
        ...
        # 只有当明确传递了role_ids时才更新角色
        if 'role_ids' in form_data:
    ...
    def get_sys_user_vo(user_id):
        ...
        return {
            ...
            'status': user.status,
            'roles': [
                {'id': r.id, 'code': r.code, 'name': r.name, 'sort': r.sort, 'remark': r.remark, 'status': r.status} for
                r in user.roles.all()]
        }, None
    ...
    def page_sys_user(query_params):
        ...
        for user in pagination.items:
            items.append({
                ...
                'status': user.status,
                'roles': [
                    {'id': r.id, 'code': r.code, 'name': r.name, 'sort': r.sort, 'remark': r.remark, 'status': r.status}
