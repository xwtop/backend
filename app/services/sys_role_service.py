...
from app.models import SysRole, SysRolePermission
...
class SysRoleService:
    ...
    def get_sys_role_vo(role_id):
        ...
        return {
            ...
            'status': role.status,
            'permissions': [
                {'id': p.id, 'code': p.code, 'name': p.name, 'resource': p.resource, 'action': p.action, 'type': p.type,
                 'remark': p.remark, 'status': p.status} for p in role.permissions.all()]
        }, None
    ...
    def page_sys_role(query_params):
        ...
        for role in pagination.items:
            items.append({
                ...
                'remark': role.remark,
                'status': role.status,
                'permissions': [{'id': p.id, 'code': p.code, 'name': p.name, 'resource': p.resource, 'action': p.action,
                                 'type': p.type, 'remark': p.remark, 'status': p.status} for p in
